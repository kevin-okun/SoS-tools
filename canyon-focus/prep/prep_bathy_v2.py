#!/usr/bin/env python3
"""
prep_bathy_v2.py — Phase 1 bathymetry prep for the Blacks canyon-focusing tool.
...
CANYON_SEEDS = [
    ...
    (476200.0, 3635600.0),  # La Jolla branch, near head
...
def build_fill_surface(z1m, transform1m):
    ...
    for e, n in CANYON_SEEDS:
        ...
        if labels[i, j] == 0:
            print(f"WARNING: canyon seed E{e:.0f} N{n:.0f} not inside closing "
                  f"mask; check seeds")
        else:
            keep.add(labels[i, j])
...
def make_figures(z5, zf5, t5):
    ...
    fig.savefig(os.path.join(FIG_DIR, "bathy_checkpoint_maps_v2.png"), dpi=140)
    plt.close(fig)
    ...
    fig.savefig(os.path.join(FIG_DIR, "bathy_checkpoint_sections_v2.png"), dpi=140)
    plt.close(fig)
...
def main():
    ...
    write_geotiff(z5, t5, os.path.join(OUT_DIR, "blacks_real_5m_msl_v2.tif"))
    write_geotiff(zf5, t5, os.path.join(OUT_DIR, "blacks_filled_5m_msl_v2.tif"))

    zm, zfm, tm = grids["model"]
    zr = write_celeris_bathy(zm, os.path.join(OUT_DIR, "bathy_real_v2.txt"))
    zc = write_celeris_bathy(zfm, os.path.join(OUT_DIR, "bathy_filled_v2.txt"))
    ...
    for k in ("run_example", "exampleDirs"):
        config.pop(k, None)
    with open(os.path.join(OUT_DIR, "config_base_v2.json"), "w") as fh:
